...
Use this skill when fixing, editing, changing, debugging, or working with any code, in any programming language.
...
function process(data, results, applyFlag = false):
  for item in data:
    if item > 0:
      if applyFlag:
        append item * 1.0825 to results   # tax
      else:
...
function processPositiveValues(values, applyTax = false):
  rate = applyTax ? 1 + TAX_RATE : 1
  return filter(values, item > 0).map(item -> item * rate)
```
...
When reviewing code:
1. Review with the full clean-code checklist in mind
